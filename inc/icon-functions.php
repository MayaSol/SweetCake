 ...
 */
function sweetcake_include_svg_icons() {
 ...
 */
function sweetcake_get_svg( $args = array() ) {
//  error_log("sweetcake_get_svg__start");
//  $my_arr = print_r($args, true);
//  error_log("args = " . $my_arr);
	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', 'sweetcake' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', 'sweetcake' );
	}
	 ...
	 *
	 * Example 1 with title: <?php echo sweetcake_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ) ) ); ?>
	 *
	 * Example 2 with title and description: <?php echo sweetcake_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
 ...
 */
function sweetcake_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'sweetcake_nav_menu_social_icons', 10, 4 );
 ...
 */
function sweetcake_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {

	if ( 'top' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . sweetcake_get_svg( array( 'icon' => 'angle-down' ) );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'sweetcake_dropdown_icon_to_menu_link', 10, 4 );
 ...
 */
function sweetcake_social_links_icons() {
